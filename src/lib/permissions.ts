...
export const PERMISSION_GROUPS: { group: string; perms: { key: string; label: string }[] }[] = [
  ...
  },
  {
    ...
    perms: [
      ...
      { key: "orders.confirm", label: "تأكيد الأوردرات تليفونيًا" },
  ...
  {
    group: "لوحة تحليلات الأعمال",
    // صلاحية مستقلة (مش جزء من "reports.view") لأن الصفحة دي بتجمع في مكان واحد بيانات حساسة
    // من كل قسم في الشركة مع بعض (ربح، أداء موظفين بالاسم، عملاء، مبيعات) - مش كل حد عنده
    // "عرض التقارير" العادي المفروض يشوف الصورة الكاملة دي بالضرورة. افتراضيًا الأدمن بس عنده
    // الصلاحية دي (زي أي صلاحية تانية، الأدمن عنده كل حاجة تلقائيًا) - وتقدر تضيفها لأي دور تاني
    // من شاشة "المستخدمون والصلاحيات" لو حابب موظف/محاسب معين يشوفها.
    perms: [{ key: "analytics.view", label: "لوحة تحليلات الأعمال الشاملة" }],
  },
  {
    group: "الإعدادات",
